"""escreva um programa que faça o computador 'pensar' em um numero inteiro entre 0 e 5 e peça
para o usuario tentar descobrir qual foi o numero escolhido pelo computador.
o programa deverá escvrever na tela se o usuario venceu ou perdeu"""
import random
list = [0,1,2,3,4,5]
n1 = random.choice(list)
resposta = str(input('Qual o numero que eu pensei?'))
print('Eu pensei no numero {}'.format(n1))

if  n1 == resposta:
    print('Parabens!Você acertou')
else:
    print("ah você errou, que pena...")
